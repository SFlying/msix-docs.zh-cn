...
ms.locfileid: "89089815"
---
# <a name="create-an-app-installer-file-manually"></a>手动创建应用安装程序文件

本文介绍如何手动创建用于定义 [相关集](../package/optional-packages.md)的应用安装程序文件。 相关集不是一个实体，而是主要包和可选包的组合。 
...
在部署期间，会根据 `Uri` 元素中引用的应用包来验证应用安装程序文件。 因此，`Name`、`Publisher` 和 `Version` 应与应用程序包清单中的 [Package/Identity](/uwp/schemas/appxpackage/uapmanifestschema/element-identity) 元素匹配。

## <a name="how-to-create-an-app-installer-file"></a>如何创建应用安装程序文件

若要作为一个实体来分配相关集，你必须创建一个应用安装程序文件，其中包含该 [appinstaller 架构](/uwp/schemas/appinstallerschema/app-installer-file)所需的元素。
...
如果主应用包为 .msixbundle 或 .appxbundle 或文件，则使用代替，如下 `<MainBundle>` `<MainPackage>` 所示。 对于捆绑包，ProcessorArchitecture 不是必需的。
...
`<MainBundle>` 或 `<MainPackage>` 属性中的信息应该分别与应用程序包清单或应用包清单中的 [Package/Identity](/uwp/schemas/appxpackage/uapmanifestschema/element-identity) 元素匹配。
...
应用安装程序文件还可以指定更新设置，以便在发布较新的应用安装程序文件时可以自动更新相关集。 **<UpdateSettings>** 是一个可选元素。 在  **<UpdateSettings>** OnLaunch 选项中，指定应在应用启动时进行更新检查，HoursBetweenUpdateChecks = "12" 指定应每隔12小时执行一次更新检查。 如果未指定 HoursBetweenUpdateChecks，则用于检查更新的默认时间间隔为 24 小时。 其他类型的更新，如后台更新，可以在 "更新设置"[架构](/uwp/schemas/appinstallerschema/element-update-settings)中找到;在 OnLaunch[架构](/uwp/schemas/appinstallerschema/element-onlaunch)中可以找到其他类型的启动更新，如使用提示更新的更新。
...
有关 XML 架构的所有详细信息，请参阅[应用安装程序文件参考](/uwp/schemas/appinstallerschema/app-installer-file)。

> [!NOTE]
> 应用安装程序文件类型是 Windows 10 版本 1709 (windows 10 秋季创建者更新) 中的新类型。 在以前版本的 Windows 10 上，不支持使用应用安装程序文件部署 Windows 10 应用。 从 Windows 10 版本1803开始， **HoursBetweenUpdateChecks** 元素可用。