---
Description: 为现有 Windows 窗体、WPF 或 Win32 应用或游戏创建现代 Windows 应用包。 为 Windows 10 用户添加现代体验, 并简化部署和盈利。
title: 打包桌面应用程序
ms.date: 07/29/2019
ms.topic: article
...
|  Tool  | 用于创建包的支持的操作系统版本  | 已安装包的支持的操作系统版本  |  支持的包格式  |
|-----------------------------|-----------------------------------------------|-----------------------------------------------|-----------------------------|
...
:heavy_check_mark:自动生成视觉资产。

:heavy_check_mark:使用可视化设计器对清单进行更改。

:heavy_check_mark:使用向导生成您的包。
...
如果使用 DAC 打包应用程序, 则可以使用新``-Verify``标志根据打包的桌面应用程序和存储要求来验证包。 请参阅[打包应用, 对应用进行签名, 并准备好应用商店提交](desktop-to-uwp-run-desktop-app-converter.md#optional-parameters)。
