...
从高层次上看，将应用的部分文件拆分到其他包（非体系结构程序包）后，你将无法直接访问那些相对于你的代码运行位置的文件。 原因在于这些软件包跟你的体系结构程序包安装到了不同的目录。 例如, 如果您正在玩游戏, 并且您的游戏已本地化为法语和德语, 并且您是为 x86 和 x64 计算机而构建的, 则应在游戏的应用捆绑包中安装这些应用包文件:
...
当你的游戏安装到用户的计算机时, 每个应用包文件在**WindowsApps**目录中都有其自己的文件夹。 因此，对于运行 64 位 Windows 的法国用户，你的游戏将如下所示：
